...
it('use --list-runners shows registered runners', () => {
  const r = execSync(`node ${CLI} use --list-runners`, { encoding: 'utf8', env: { ...process.env, KDNA_QUIET: '1' } });
  assert.ok(r.includes('mock:default'));
  assert.ok(r.includes('cli:default'));
});

it('use with mock runner produces JSON result', () => {
  const r = execSync(`node ${CLI} use ${FIXTURE} --task="Deploy risk assessment" --runner mock:default --as json`, { encoding: 'utf8', env: { ...process.env, KDNA_QUIET: '1' } });
...
it('use --as=trace produces trace output', () => {
  const r = execSync(`node ${CLI} use ${FIXTURE} --task="Review code" --runner mock:default --as trace`, { encoding: 'utf8', env: { ...process.env, KDNA_QUIET: '1' } });
...
it('use --plan-only produces plan without execution', () => {
  const r = execSync(`node ${CLI} use ${FIXTURE} --task="Test" --plan-only --as json`, { encoding: 'utf8', env: { ...process.env, KDNA_QUIET: '1' } });
  const plan = JSON.parse(r);
  assert.strictEqual(plan.plan_version, '0.9.0');
  assert.strictEqual(plan.mode, 'single');
  assert.strictEqual(plan.runner, null, 'plan-only should not set runner');
});

it('use --dry-run validates runner without execution', () => {
  const r = execSync(`node ${CLI} use ${FIXTURE} --task="Dry run test" --runner mock:default --dry-run --as json`, { encoding: 'utf8', env: { ...process.env, KDNA_QUIET: '1' } });
  const out = JSON.parse(r);
...
it('use with non-existent asset fails', () => {
  try {
    execSync(`node ${CLI} use /nonexistent/path.kdna --task="test" --runner mock:default --as json`, { encoding: 'utf8', env: { ...process.env, KDNA_QUIET: '1' } });
    assert.fail('Should have thrown');
  } catch (e) {
    assert.ok(e.status !== 0);
  }
