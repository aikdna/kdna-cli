...
function cmdClusterValidate(target) {
  ...
  console.log(JSON.stringify({
    ...
    primary_candidates: (manifest.domains || []).filter(d => d.role === 'primary-candidate').length,
...
function cmdClusterInfo(target) {
  ...
  if (!manifest.cluster_id) error('Not a valid cluster manifest (missing cluster_id)', EXIT.INPUT_ERROR);
  ...
  console.log(`  Format:           ${manifest.format || 'kdna-cluster'} v${manifest.format_version || '?'}`);
...
function cmdClusterPlanUse(target, args) {
  if (!target) error('Usage: kdna cluster plan-use <kdna.cluster.json> --task="..."', EXIT.INPUT_ERROR);
  const abs = path.resolve(target);
  const manifest = loadManifest(abs);
  if (!manifest) error(`Not found: ${target}`, EXIT.INPUT_ERROR);
  ...
  const validation = validateClusterManifest(manifest);
  if (!validation.valid) {
    error(`Cluster manifest invalid:\n${validation.errors.map(e => `  - ${e}`).join('\n')}`, EXIT.VALIDATION_FAILED);
  }
...
function cmdClusterConflicts(target, args) {
  if (!target) error('Usage: kdna cluster conflicts <kdna.cluster.json> --task="..."', EXIT.INPUT_ERROR);
  const abs = path.resolve(target);
  const manifest = loadManifest(abs);
  if (!manifest) error(`Not found: ${target}`, EXIT.INPUT_ERROR);
...
function cmdClusterMigrate(target, args) {
  if (!target) error('Usage: kdna cluster migrate <legacy.json> --from=<format> [--out=<path>]', EXIT.INPUT_ERROR);
  ...
  if (!args.includes('--from')) {
    if (legacy.packages && Array.isArray(legacy.packages)) {
      // No need to change - already set from default
  ...
  if (outPath) {
    ...
    console.log(`Migrated manifest written to: ${outPath}`);
    console.log(`Warnings: ${report.warnings.length}`);
    console.log(`Manual decisions required: ${report.manual_decisions_required.length}`);
    if (report.manual_decisions_required.length) {
