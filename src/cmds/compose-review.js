...
function cmdComposeReview(args) {
  ...
  if (!command || args.includes("--help") || args.includes("-h")) {
    process.stderr.write(
      "Usage:\n" +
        ...
        "  --consumer-index=<path>   Consumer index to update\n" +
        "  --force                   Skip validation gate (dangerous)\n"
    );
  ...
  if (command === "compose-review-workbook") {
    if (!target) error("Usage: kdna compose-review-workbook <diagnostic.json> [--out=<path>] [--as=<json|md>]", EXIT.INPUT_ERROR);
    cmdWorkbook(target, args);
    return;
  }

  if (command === "validate-compose-decisions") {
    if (!target) error("Usage: kdna validate-compose-decisions <ledger.jsonl> [options]", EXIT.INPUT_ERROR);
    cmdValidateDecisions(target, args);
    return;
  }

  if (command === "apply-reviewed-compose-decisions") {
    if (!target) error("Usage: kdna apply-reviewed-compose-decisions <ledger.jsonl> [options]", EXIT.INPUT_ERROR);
...
function cmdWorkbook(target, args) {
  ...
  const output = as === "json"
    ...
    : formatWorkbookMarkdown(workbook);
...
function buildWorkbook(diagnostic) {
  ...
  return {
    ...
    candidateSidecarPatch: candidatePatch.length > 0 ? candidatePatch : [{ note: "no primary domain resolved — patch generation skipped" }],
  };
}
...
function generateReviewPrompts(decisions, diagnostic) {
  ...
  const blockedGates = decisions.filter(
    (d) => d.status === "BLOCKED" || d.status === "FAIL"
  );
...
function formatWorkbookMarkdown(workbook) {
  ...
  for (const d of workbook.decisions) {
    ...
    if (d.details.primary) {
      lines.push(`- **Primary:** ${typeof d.details.primary === "string" ? d.details.primary : d.details.primary.domain_id || "?"}`);
    }
...
function cmdValidateDecisions(target, args) {
  ...
  const gates = gatesRaw.split(",").map((s) => s.trim()).filter(Boolean);
  ...
  for (const record of records) {
    ...
    for (const mode of REPLAY_MODES) {
      ...
      const currentRun = {
        ...
        results: gateResults.map((g) => ({
          ...
          score:
            g.score != null
              ? g.score
              : g.pass === true ? 1.0 : g.pass === false ? 0.0 : 0.5,
          pass: g.pass === true,
      ...
      const modePassed = gateResults.every(
        (g) => g.pass === true || g.pass === null
      );

      replay[mode] = {
        overall: modePassed ? "pass" : "fail",
        passed: modePassed,
        failed_gates: gateResults
...
function cmdApplyDecisions(target, args) {
  ...
  if (records.length === 0) {
    console.log(JSON.stringify({ applied: [], skipped: [], summary: { total: 0, applied: 0, skipped: 0 } }, null, 2));
    return;
  }
  ...
  for (const record of records) {
    ...
    const validationResult = validationReport.results.find(
    ...
    if (record.source === "sealed-derived") {
      skipped.push({
        record_id: recordId,
        reason: "sealed-derived evidence cannot be auto-applied — requires human review (--force cannot override)",
      });
      continue;
    }
    ...
    const existingIdx = consumerIndex.entries.findIndex(
      (e) => e.domain_id === primaryDomain
    );
